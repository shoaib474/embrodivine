        ...
        <div className="space-y-6">
          {mainServices.map((service) => {
            ...
            return (
              <div
                key={service.id}
                className="rounded-2xl border border-gray-200 bg-white overflow-hidden transition-all duration-500"
              >
                {/* HEADER */}
                <button
                  onClick={() => setActive(isOpen ? null : service.id)}
                  className="w-full flex flex-col md:flex-row gap-6 p-6 text-left hover:bg-[#F5F7FA] transition"
                >
                  {/* IMAGE (ONLY MAIN SERVICE) */}
                  <div className="md:w-40 w-full h-40 flex-shrink-0 rounded-xl overflow-hidden border border-gray-200">
                    <img
                      src={service.image}
                      alt={service.title}
                  ...
                  {/* ICON */}
                  <div className="text-[#007BFF] text-3xl self-start md:self-center">
                    {isOpen ? "−" : "+"}
                  </div>
                </button>
                ...
                >
                  <div className="px-6 pb-6 grid md:grid-cols-2 gap-6">
                    {service.services.map((sub, idx) => (
                      ...
                      >
                        <h4 className="text-lg font-semibold text-[#007BFF] mb-3">
                          {sub.name}
                        </h4>
                        ...
                        <ul className="space-y-2">
                          {sub.features.map((f, i) => (
                            <li
                              key={i}
                              className="flex items-center gap-2 text-[#333333] text-sm"
                            >
                              <CheckCircle className="w-4 h-4 text-[#007BFF] flex-shrink-0" />
                              {f}
                            </li>
                          ))}
                        </ul>
